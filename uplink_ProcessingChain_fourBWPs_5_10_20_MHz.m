%% first run the script "downlink_ProcessingChain_fourBWPs_5_10_20_MHz.m"
%% Operation for the 1st BWP

% NCO Operation for 1st BWP
for ii = 1:length(ps_out_5MHz)
...
figure
plot(linspace(-fs_up_5MHz/2,fs_up_5MHz/2,length(waveform_decim_1stBWP)),20*log10(abs(fftshift(fft(waveform_decim_1stBWP)))))
...
fs_up_10MHz = 15.36;
waveform_decim_2ndBWP = decimate(NCO_up_2ndBWP,32);
figure
plot(linspace(-fs_up_10MHz/2,fs_up_10MHz/2,length(waveform_decim_2ndBWP)),20*log10(abs(fftshift(fft(waveform_decim_2ndBWP)))))
...
fs_up_20MHz = 30.72;
waveform_decim_3rdBWP = decimate(NCO_up_3rdBWP,16);
figure
plot(linspace(-fs_up_20MHz/2,fs_up_20MHz/2,length(waveform_decim_3rdBWP)),20*log10(abs(fftshift(fft(waveform_decim_3rdBWP)))))
...
figure
plot(linspace(-fs_up_5MHz/2,fs_up_5MHz/2,length(waveform_decim_4thBWP)),20*log10(abs(fftshift(fft(waveform_decim_4thBWP)))))
